...
const tickerReviews2 = [...reviewsRow2, ...reviewsRow2, ...reviewsRow2];
...
// Componente modificado para o mural (sem foto, sem estrela)
const ReviewCard = ({ review }: { review: Review }) => (
  <div className="h-fit flex-shrink-0 w-[280px] md:w-[320px] py-3 px-5 rounded-2xl bg-white/10 backdrop-blur-md border border-zinc-100/60 shadow-sm hover:border-emerald-200/80 transition-colors select-none">
    <p className="text-[13px] md:text-sm text-zinc-800 italic leading-snug mb-2 pointer-events-none">
      "{review.text}"
    </p>
...
const TickerRow = ({
  ...
  return (
    ...
    >
      <motion.div
        ref={containerRef}
        className="flex items-center gap-4 pl-4 w-max will-change-transform"
        style={{ x }}
        drag="x"
      ...
      >
        {reviews.map((review, i) => (
          <ReviewCard key={i} review={review as Review} />
        ))}
      </motion.div>
    </div>
  );
    ...
    <section id="wall-of-love" data-section="wall-of-love" className="bg-white py-16 md:py-24 overflow-hidden relative flex flex-col">
      ...
      <div className="relative w-full py-20 mt-4 mb-12 overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col gap-4 w-full scale-105">
          ...
          <TickerRow reviews={tickerReviews2} direction="right" speed={1.75} />
        </div>

        {/* Fade Edges para efeito infinito */}
        ...
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-2 opacity-40 md:hidden pointer-events-none z-20">
          ...
          <div className="flex gap-1">
            ...
            <div className="w-1 h-1 rounded-full bg-zinc-400 opacity-25" />
          </div>
        </div>
      </div>
